...
router.get('/clientes', (req, res) => {
  try {
    ...
    const usuarios = getUsuarios();
    const idxG = usuarios.findIndex(u => u.id === gestoria.id);
    if (idxG !== -1) {
      if (!usuarios[idxG].accesos) usuarios[idxG].accesos = [];
      usuarios[idxG].accesos.push(new Date().toISOString());
      if (usuarios[idxG].accesos.length > 50) usuarios[idxG].accesos = usuarios[idxG].accesos.slice(-50);
      saveUsuarios(usuarios);
    }
    const ges = usuarios[idxG] || {};
