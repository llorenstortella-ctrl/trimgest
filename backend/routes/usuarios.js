...
router.post('/login', async (req, res) => {
  try {
    ...
    const idxU = usuarios.findIndex(u => u.id === usuario.id);
    if (idxU !== -1) {
      if (!usuarios[idxU].accesos) usuarios[idxU].accesos = [];
      usuarios[idxU].accesos.push(new Date().toISOString());
      if (usuarios[idxU].accesos.length > 50) usuarios[idxU].accesos = usuarios[idxU].accesos.slice(-50);
      saveUsuarios(usuarios);
    }
