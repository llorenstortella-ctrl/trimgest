...
async function procesarPDFFactura(buffer, nombreArchivo, empresaId, emailUsuario, tipoFactura) {
  tipoFactura = tipoFactura || 'proveedor';
...
router.post('/inbound', express.text({ type: '*/*', limit: '25mb' }), async (req, res) => {
  res.json({ ok: true });

  try {
    let toEmail, fromEmail, adjuntos;

    // Si viene raw email del Worker de Cloudflare
    if (typeof req.body === 'string' && req.body.includes('MIME-Version')) {
      const parsed = await simpleParser(req.body);
      toEmail = (parsed.to && parsed.to.text || '').toLowerCase();
      fromEmail = (parsed.from && parsed.from.text || '').toLowerCase();
      adjuntos = (parsed.attachments || []).map(function(a) {
        return { filename: a.filename, content: a.content.toString('base64') };
      });
    } else {
      // Formato JSON (Resend o Worker con JSON)
      let payload;
    ...
    const esCli = toEmail.startsWith('clientes+');
    const esNom = toEmail.startsWith('nominas+');
    if (!esFact && !esCli && !esNom) return;
    ...
    for (var i = 0; i < pdfs.length; i++) {
      ...
      try {
        ...
        var resultado;
        if (esFact || esCli) {
          var tipoFact = esCli ? 'cliente' : 'proveedor';
          resultado = await procesarPDFFactura(buffer, adj.filename, empresaId, usuario.email, tipoFact);
        } else {
    ...
    const tipo = esNom ? 'nominas' : esCli ? 'facturas de cliente' : 'facturas de proveedor';
