...
async function extraerDatosFactura(rutaPDF) {
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `Eres un experto contable español. Analizas facturas de la empresa "${MI_EMPRESA}".
...
7. Si no hay anomalías devuelve null sin comillas`
      ...
      {
        role: 'user',
        content: `Analiza esta factura y devuelve SOLO este JSON sin texto adicional:
{
...
${textoFactura}`
  ...
  // Si la IA devuelve nuestra empresa como nombre, corregir tipo
